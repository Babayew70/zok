...
const AdminPanel = () => {
  ...
  // Cooperation state
  const [cooperationItems, setCooperationItems] = useState([]);
  ...
  // Загружаем данные при смене раздела (после первой загрузки страницы)
  useEffect(() => {
    if (!isLoaded) {
      setIsLoaded(true);
      return; // Пропускаем первую загрузку
    }

    // Проверяем авторизацию перед загрузкой данных
    const token = localStorage.getItem('adminToken');
    if (!token) return; // Не загружаем данные если не авторизован

    if (activeSection === 'products') fetchProducts();
    if (activeSection === 'categories') fetchCategories();
    if (activeSection === 'orders') fetchOrders();
    if (activeSection === 'smtp') fetchSmtpSettings();
    if (activeSection === 'reports') fetchReports();
    if (activeSection === 'banners') fetchBanners();
    if (activeSection === 'portfolio') fetchPortfolio();
    if (activeSection === 'reviews') fetchAdminReviews();
    if (activeSection === 'cooperation') fetchCooperation();
    if (activeSection === 'pdfcatalog') fetchCatalogs();
    if (activeSection === 'logs') fetchLogs();
    if (activeSection === 'dashboard') {
      // Для дашборда загружаем всё
      fetchOrders();
      fetchReports();
      fetchPortfolio();
      fetchAdminReviews();
  ...
  // Katalog Zok pdf fayl
  const handleCatalogUpload = async () => {
    console.log('Button clicked, file:', catalogFileRef.current);
    ...
    const file = catalogFileRef.current;
    ...
    formData.append('title', 'Каталог ZOK');

    const token = localStorage.getItem('adminToken');
    if (!token) {
      alert('Вы не авторизованы! Войдите в админку.');
      return;
    }

    try {
      console.log('Sending request to:', `${API_URL}/catalog/upload`);
      const response = await axios.post(`${API_URL}/catalog/upload`, formData, {
        headers: {
  ...
  const handleDeleteCatalog = async (id) => {
    ...
    try {
      await axios.delete(`${API_URL}/catalog/${id}`, getAuthHeaders());
  ...
  const fetchCatalogs = async () => {
    try {
      const response = await axios.get(`${API_URL}/catalog/all`, getAuthHeaders());
  ...
  const fetchAdminReviews = async () => {
    ...
    if (!token) {
      setReviewsError('Вы не авторизованы. Пожалуйста, войдите в админ-панель.');
      return;
    }
    ...
    try {
      ...
      const errorMsg = error.response?.data?.error || error.message || 'Ошибка при загрузке отзывов';
      setReviewsError(errorMsg + ' (код: ' + error.response?.status + ')');
    }
  };
  ...
  const fetchCooperation = async () => {
    try {
      const response = await axios.get(`${API_URL}/cooperation`, getAuthHeaders());
      setCooperationItems(response.data);
    } catch (error) {
      console.error('Error fetching cooperation:', error);
    }
  };

  const handleDeleteCooperation = async (id) => {
    if (!window.confirm('Удалить эту заявку?')) return;
    try {
      await axios.delete(`${API_URL}/cooperation/${id}`, getAuthHeaders());
      fetchCooperation();
    } catch (error) {
  ...
  const handleUpdateCooperationStatus = async (id, status) => {
    try {
      await axios.put(`${API_URL}/cooperation/${id}`, { status }, getAuthHeaders());
      fetchCooperation();
    } catch (error) {
      ...
      <div className="admin-panel-layout">
        ...
        <aside className={`admin-sidebar ${sidebarOpen ? 'open' : ''}`}>
          ...
          <nav className="sidebar-nav">
            ...
            <button className={`sidebar-link ${activeSection === 'pdfcatalog' ? 'active' : ''}`} onClick={() => { setActiveSection('pdfcatalog'); fetchCatalogs(); setSidebarOpen(false); }}>
              <FaFilePdf /> PDF Каталог
            </button>
            ...
            <button className={`sidebar-link ${activeSection === 'cooperation' ? 'active' : ''}`} onClick={() => { setActiveSection('cooperation'); setSidebarOpen(false); }}>
              <FaHandshake /> Сотрудничество
            </button>
        ...
        <main className="admin-main">
          ...
          {activeSection === 'pdfcatalog' && (
            <section className="catalog-admin-section">
              ...
              <div className="catalog-upload-card">
                ...
                <div className="upload-controls">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      catalogFileRef.current = file;
                      setSelectedFileName(file ? file.name : '');
                    }}
                    style={{ display: 'none' }}
                  />
                  <label htmlFor="catalog-upload-input" className="file-select-btn">
                    {selectedFileName ? '📄 ' + selectedFileName : '📎 Выбрать файл'}
                  </label>
                  <button
                ...
                {selectedFileName && (
                  <p style={{ marginTop: '10px', opacity: 0.8 }}>✅ Файл выбран: {selectedFileName}</p>
                )}
              </div>

              {/* Текущий каталог */}
              <div className="current-catalog-card">
                  ...
                  catalogs.map(cat => (
                    <div key={cat.id} className="catalog-item">
                      <div className="catalog-info">
                        ...
                        <div>
                          <span className="catalog-name">{cat.file_name || 'Каталог ZOK'}</span>
                      ...
                      <div className="catalog-actions">
                        <a
                          href={cat.file_url.startsWith('http') ? cat.file_url : `http://localhost:5001${cat.file_url}`}
                          target="_blank"
          ...
          {activeSection === 'reviews' && (
            <section>
              ...
              {reviewsError && (
                <div className="error-state" style={{ padding: '2rem', background: '#fee', borderRadius: '8px', marginBottom: '1rem' }}>
                  <p style={{ color: '#c00', fontWeight: 'bold' }}>❌ Ошибка: {reviewsError}</p>
                  <p style={{ fontSize: '0.9rem', color: '#666' }}>Проверьте консоль браузера (F12) для деталей</p>
                </div>
              )}
              {adminReviews.length === 0 && !reviewsError ? (
                <div className="empty-state">
                ...
                <div className="reviews-moderation">
                  {adminReviews.map(review => (
                    <div key={review.id} className={`review-mod-card ${review.is_approved ? 'approved' : 'pending'}`}>
                      ...
                      <p className="review-mod-text">{review.text_content}</p>
          ...
          {activeSection === 'cooperation' && (
            <section>
              ...
              {cooperationItems.length === 0 ? (
                <div className="empty-state">
                  <FaHandshake style={{ fontSize: '3rem', opacity: 0.3 }} />
                  <p>Нет заявок на сотрудничество</p>
                </div>
              ) : (
                <div className="cooperation-list">
                  {cooperationItems.map(item => (
                    <div key={item.id} className="cooperation-card">
                      <div className="cooperation-header">
                        <strong>{item.name || 'Без названия'}</strong>
                        <span className={`status-badge status-${item.status}`}>
                          {item.status === 'new' ? 'Новый' : item.status === 'contacted' ? 'Связались' : item.status === 'completed' ? 'Завершено' : item.status === 'rejected' ? 'Отклонено' : item.status}
                        </span>
                      </div>
                      <div className="cooperation-info">
                        <p><strong>Телефон:</strong> {item.phone}</p>
                        <p><strong>Тип партнёра:</strong> {item.partner_type === 'shop' ? 'Магазин' : item.partner_type === 'master' ? 'Мастер' : item.partner_type === 'designer' ? 'Дизайнер' : item.partner_type}</p>
                        <p><strong>Дата:</strong> {new Date(item.created_at).toLocaleDateString()}</p>
                      </div>
                      {item.notes && (
                        <div className="cooperation-message">
                      ...
                      <div className="cooperation-actions">
                        <select
                          value={item.status}
                          onChange={(e) => handleUpdateCooperationStatus(item.id, e.target.value)}
                        ...
                        <button
                          className="delete-btn"
                          onClick={() => handleDeleteCooperation(item.id)}
                          title="Удалить"
                        >
                          <FaTrash />
                        </button>
                ...
                <div className="modal-overlay" onClick={() => setShowBannerModal(false)}>
                  <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                    ...
                    <form onSubmit={handleBannerSubmit}>
                      <div className="form-group">
                        <label>📱 Изображение для телефона</label>
                        <FileUploader type="banners" label="📷 Загрузить баннер для мобильного" onUpload={(url) => setBannerForm({ ...bannerForm, image_url_mobile: url })} />
                        <input
                          type="text"
                          name="image_url_mobile"
                          value={bannerForm.image_url_mobile}
                          onChange={(e) => setBannerForm({ ...bannerForm, image_url_mobile: e.target.value })}
                          placeholder="URL или загрузите (1080x1920)"
                        />
                      </div>
                      <div className="form-group">
                        <label>🖥️ Изображение для компьютера *</label>
